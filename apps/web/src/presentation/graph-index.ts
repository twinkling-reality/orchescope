...
export interface GraphIndex {
  ...
  /**
   * True when this report carries a join, so `neverExercised` is the join's own set rather than a
   * guess. Without one, a part with no runtime evidence cannot be told apart from a kind no trace
   * would ever record, and presence has to give the weaker answer.
   */
  readonly joinComputed: boolean;
...
export function buildGraphIndex(bundle: ReportBundle): GraphIndex {
  ...
  return {
    ...
    degreeByComponent,
    hasRuntimeEvidence,
    joinComputed: declaredNotExercised !== null,
