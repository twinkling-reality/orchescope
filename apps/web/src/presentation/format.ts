...
/**
 * A unit that is a bare count says nothing the number beside it has not already said, and the metric
 * carries its own name, so `count` is dropped rather than printed. `39 count` is not a measurement
 * anybody reads.
 */
const BARE_COUNT_UNITS = new Set(['count', 'counts']);

/**
 * Agrees a noun unit with the value in front of it.
 *
 * The cached bundles carry `occurrence`, `component` and `tool` in the singular and `tokens` and
 * `runs` in the plural, over 108 of the 126 units in the corpus, so both directions are needed:
 * without this `crewai` printed `6 occurrence` and `6 component` on its Findings screen, which is
 * the same class of slip as the engine's `run(s)` recorded in `docs/design/TODO.md`.
 *
 * Only the last word is inflected, so a unit that is already a phrase keeps its shape: `tool calls`
 * stays `tool calls`.
 */
function agreeWithValue(value: number, unit: string): string {
  ...
  words[words.length - 1] = Math.abs(value) === 1 ? singular : `${singular}s`;
  return words.join(' ');
}
...
export function formatMetricValue(value: number, unit: string): string {
  ...
  if (BARE_COUNT_UNITS.has(normalised)) {
    return formatNumber(value);
  }
  return `${formatNumber(value)} ${agreeWithValue(value, unit)}`;
}
