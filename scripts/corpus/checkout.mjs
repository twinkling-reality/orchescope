 ...
 *
 * Third party source is never vendored into this repository. A full corpus run clones a git entry at its pinned
 * commit into a directory git ignores. The required run materialises only explicitly selected entries from bounded
 * source archives pinned by normalized tree and licence digests. Both forms keep source outside the distribution.
 *
...
import { materializeRequiredArchive } from './archive.mjs';
...
const auditRootFor = (directory, entry) => {
...
const checkoutGit = (root, entry, allowNetwork) => {
  ...
  return auditRootFor(directory, entry);
};

const checkoutRequiredArchive = async (root, entry) => {
  const directory = directoryFor(root, `${entry.name}-required`);
  await materializeRequiredArchive(entry, directory);
  return auditRootFor(directory, entry);
};
...
export const checkout = (root, entry, acquisition) => {
  if (entry.source === 'local') return checkoutLocal(root, entry);
  if (acquisition === 'required_archive') return checkoutRequiredArchive(root, entry);
  return checkoutGit(root, entry, acquisition === 'git');
};
