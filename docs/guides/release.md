...
an exact `browser_use.Agent` construction disappeared and its candidate run call received no bounded refusal even
though every supported Python file parsed, and the terminal document claimed that no agent system was present without
a bounded refusal. The positive is a
permanent source-cited regression, the negative adds no duplicate precision entry, and neither lineage can be reused as
an unseen holdout.

The [blocked 63f31253 evaluation](../research/63f31253-blocked-blind-evaluation.md) records a ninth completed block:
an exact `agentflow.core` Agent, ToolNode and cyclic compiled graph population disappeared while every supported source
file parsed, and the terminal document claimed that no agent system was present without a bounded refusal. The positive
...
The [0.9.1 acceptance holdouts](../research/0.9.1-acceptance-used-holdouts.md) record a post-publication check that
exposed a silent imported construction. The used positive is pinned as an `agent_system` whose scan stays
`agentSystemDetected: false` and requires a source-located `unclaimed_imported_construction`. The used negative
