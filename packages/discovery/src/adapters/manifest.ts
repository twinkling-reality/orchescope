...
export const manifestAdapter: AgentSystemAdapter = {
  ...
  version: '1',
  ecosystem: 'manifest',
  appliesTo: (context) => context.configs.some((document) => MANIFEST_PATHS.includes(document.path)),
  discover: (context, builder): AdapterFindings => {
    ...
    for (const [index, declared] of manifest.components.entries()) {
      ...
      builder.addComponent({
        ...
        configLocations: [{ file: document.path, pointer }],
        evidence: [
          configEntryEvidence({
            producer: ADAPTER_ID,
            location: { file: document.path, pointer },
      ...
      components += 1;
      context.bindings.register(document.path, declared.name, identity);
    }

    for (const [index, declared] of manifest.edges.entries()) {
      const from = resolveEndpoint(manifest, context, document.path, declared.from);
      const to = resolveEndpoint(manifest, context, document.path, declared.to);
    ...
    return { componentsFound: components, edgesFound: edges, filesInspected: 1 };
  },
};
