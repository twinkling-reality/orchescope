...
describe('the fact cache', () => {
  it('keys branch-path, nested-call, binding-scope, parameter-default and base-class facts by analyzer version', () => {
    assert.equal(ANALYZER_VERSION, '26');
    assert.match(
      cacheKey(
        {
          path: 'src/graph.py',
